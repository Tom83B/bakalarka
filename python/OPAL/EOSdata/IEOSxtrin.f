...
cc      parameter (mx=5,mv=10,nr=169,nt=191)
cc      character*3 fixedTP
cc      data fixedTP/'yes'/  ! 'yes' for fixed T6,P; 'no' for fixed T6,rho
cc      common/eeos/esact,eos(mv)
cc      iorder=9  ! gives all 1st and 2nd order data. See instructions
c                  in esac.
c     NOTE: irad=0 does not add radiation; irad=1 adds radiation
cc      irad=0     ! does not add radiation  corrections
cc      x=0.7
cc      ztab=0.02
cc      t6=1.
cc      r=0.001
cc      p=1.133566618e-1
cc      reteos=eos
cc      if (fixedTP .eq. 'yes' ) then
cc      r=rhoofp (x,ztab,t6,p,irad)  ! calculate density (r) for fixed P.
cc      endif
cc      write (*,*) r
cc      call esac(x,ztab,t6,r,iorder,irad,reteos) ! calc EOS; use r from rhoofp
cc      write (*,'("eos=",9e14.4)') (eos(i),i=1,9)
c                                            if fixedTP='yes'
cc      end
...
c..... read  tables
      call system ('gunzip
     x /home/tom/Documents/skola/bakalarka/python/EOS/EOSdata.gz')
       open(2, FILE=
     x '/home/tom/Documents/skola/bakalarka/python/EOS/EOSdata')
